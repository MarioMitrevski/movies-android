...
class GetMovieDetailsUseCaseTest {
    ...
    fun setup() {
        ...
        getMovieDetailsUseCase = GetMovieDetailsUseCase(movieRepository, Dispatchers.Default)
    }
    ...
    fun `invoke returns success with movie details and full poster path`() = runTest {
        ...
        coEvery { movieRepository.getMovieDetails(movieId) } returns Result.Success(movie)
        ...
        val resultMovie = (result as Result.Success).data
        assertEquals("Test Movie", resultMovie.title)
        assertEquals("Test Description", resultMovie.description)
        assertEquals(BuildConfig.IMAGES_BASE_URL + "/test.jpg", resultMovie.posterPath)
    }
